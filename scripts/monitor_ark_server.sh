...
UPDATE_WINDOW_MINIMUM_HOUR=${UPDATE_WINDOW_MINIMUM_HOUR:-0} # Default to 0 if not set
UPDATE_WINDOW_MAXIMUM_HOUR=${UPDATE_WINDOW_MAXIMUM_HOUR:-23} # Default to 23 if not set
...
while true; do
    ...
    if [ "${UPDATE_SERVER}" = "TRUE" ]; then
        ...
        update_window_lower_bound=$(date -d "${UPDATE_WINDOW_MINIMUM_HOUR}:00:00" +'%s')
        update_window_upper_bound=$(date -d "${UPDATE_WINDOW_MAXIMUM_HOUR}:59:59" +'%s')
